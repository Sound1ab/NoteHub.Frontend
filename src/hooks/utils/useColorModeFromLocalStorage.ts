...
export function useColorModeFromLocalStorage(): {
  ...
  const [storedColorMode, storeColorMode] = useLocalStorage(
    LOCAL_STORAGE.COLOR_MODE
  )
  const [colorMode, setColorMode] = useState(DEFAULT_COLOR_MODE)
  const [loading, setLoading] = useState(true)

  const storeAndSetColorMode = useCallback(
    (newColorMode: COLOR_MODE) => {
  ...
  useLayoutEffect(() => {
    const loadedColorMode = storedColorMode || DEFAULT_COLOR_MODE
